...
def interior_pt(func, ineq_constraints, eq_constraints_mat, eq_constraints_rhs, x0):
    ...
    current_obj_val, current_gradient, current_hessian = calculate_objective_value(
        func, ineq_constraints, x0, scale
    )

    num_inequality_constraints = len(ineq_constraints)
    current_point = x0.copy()

    path.append(current_point)
    values.append(func(current_point)[0])

    while num_inequality_constraints / scale > 1e-8:
        for _ in range(10):
            direction = determine_optimal_direction(
                current_hessian, eq_constraints_mat, current_gradient
            )
            step_size = perform_backtracking_line_search(
                func,
                current_point,
                current_obj_val,
                current_gradient,
            ...
            next_obj_val, next_gradient, next_hessian = calculate_objective_value(
                func, ineq_constraints, next_point, scale
            )
        ...
        values.append(func(current_point.copy())[0])
        scale *= 10

    return (
        current_point,
        func(current_point.copy())[0],
...
def calculate_objective_value(objective_function, inequality_constraints, point, scale):
    ...
    log_obj_val, log_gradient, log_hessian = apply_logarithmic_barrier(
        inequality_constraints, point
    )
...
def apply_logarithmic_barrier(inequality_constraints, point):
    ...
    for constraint in inequality_constraints:
        ...
        gradient_tile = np.tile(
            gradient.reshape(gradient_dim, -1), (1, gradient_dim)
        ) * np.tile(gradient.reshape(gradient_dim, -1).T, (gradient_dim, 1))
...
def perform_backtracking_line_search(
    objective_function,
    point,
    obj_val,
    gradient,
    ...
    while (
        ...
        and current_obj_val > obj_val + alpha * step_size * gradient.dot(direction)
    ):
        step_size *= beta
        current_obj_val, _, _ = objective_function(point + step_size * direction)
        iteration_count += 1
